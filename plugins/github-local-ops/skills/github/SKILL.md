...
- For top-level pull request reviews, use `github_mutation_preview` with operation `pull_request_review`; payload must include `repo` and `number`, omit `comments`, and pass `event: "COMMENT"`, `event: "APPROVE"`, or `event: "REQUEST_CHANGES"`. Use a non-empty `body` for `COMMENT` and `REQUEST_CHANGES`; `APPROVE` may include an optional body. `commitId` is optional for top-level reviews and should only be passed when deliberately pinning the review to a specific head SHA.
