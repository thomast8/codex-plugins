...
- Preview before public writes: PR comments, new inline pull request review comments, review-thread replies, review-thread resolution, PR edits, reviewer requests, issue comments, label changes, workflow dispatch, workflow reruns, and release creation.
...
- For top-level pull request reviews, use `github_mutation_preview` with operation `pull_request_review`; payload must include `repo`, `number`, `commitId`, and `event: "COMMENT"`, `event: "APPROVE"`, or `event: "REQUEST_CHANGES"`, and must omit `comments`. Use a non-empty `body` for `COMMENT` and `REQUEST_CHANGES`; `APPROVE` may include an optional body. `commitId` must be the exact PR head SHA that was reviewed.
...
- For resolving an inline review thread, use `github_mutation_preview` with operation `review_thread_resolve` and payload `repo`, `number`, and GraphQL `threadId`. Execute only after approval, then report the returned `readbackVerified`, `readback.repositoryMatches`, `readback.numberMatches`, and `readback.isResolved` evidence.
- When the user asks to send a reviewed PR back to the author, submit a formal `REQUEST_CHANGES` review through `github_mutation_preview` with operation `pull_request_review`, no `comments`, and the exact top-level review body. Do not substitute a top-level PR comment. Raw `gh pr review` is a provider-gap fallback only if the preview/execute path is unavailable after `github_identity_status`, explicit reporting, command-scoped identity where possible, and readback with `github_pr_view`.
