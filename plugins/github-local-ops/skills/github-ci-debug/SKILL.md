...
5. If failed logs are needed and no provider log tool exists, raw `gh run view <run-id> --log-failed` is a provider-gap fallback only: run `github_identity_status` first, say why the fallback is needed, avoid `gh auth switch`, and summarize the failing step plus first actionable error.
...
- Do not use naked `gh`, `gh api`, `gh pr`, `gh repo`, `gh run`, `gh workflow`, or `gh auth switch` for normal CI workflows when GitHub Local Ops exposes a provider tool.
- After any provider-gap raw `gh` fallback, read back the relevant status with `github_checks` or `github_actions_runs`.
