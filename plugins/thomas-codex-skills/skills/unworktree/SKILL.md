...
This skill is a thin coordinator around two shell scripts. **Do not re-implement their logic in inline bash** - call them and react to their exit codes. Resolve `<unworktree_skill_dir>` from this skill's installed plugin directory.
...
```bash
<unworktree_skill_dir>/scripts/list.sh "${ARG:-}"
```
...
```bash
<unworktree_skill_dir>/scripts/reclaim.sh <wt_path> <branch> <main_root> --stash-wt
```
