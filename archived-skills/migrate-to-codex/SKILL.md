...
Keep going until the selected migration is completely done: run the migrator,
inspect the report, fix migrated Codex instructions/skills/agents/MCP config,
and re-run checks without stopping to ask for confirmation of the next step. If
the user has selected a target, you may create, edit, or replace generated Codex
artifacts in that target (`AGENTS.md`, `.codex/`, `.agents/`, `~/.Codex/`, or
`~/.codex/`) without asking again. Ask before deleting files or generated
artifacts. Preserve unrelated existing Codex config entries in
`.codex/config.toml`, `~/.Codex/config.toml`, or `~/.codex/config.toml`, such as
`notify`, `projects`, `marketplaces`, or unrelated MCP servers; do not ask about
them unless they fail validation or directly conflict with the migration. Do not
edit source Claude Code files (`.claude/`, `~/.claude/`, `.mcp.json`, or
`.claude.json`), unrelated project code, secrets, or another repository.

## Migration Order

Run the migration in this order for each selected global or project source:

1. Start by using Codex's `update_plan` tool when it is available. Do not create
   `MIGRATION_TODOS.md` or any TODO file unless the user explicitly asks. The
   plan input has a `plan` array whose items each have `step` and `status`; use
   statuses `pending`, `in_progress`, and `completed`. Make the TODOs specific
   to the selected artifacts. Before finishing, update the plan so every finished
   step is marked `completed` and no step remains `in_progress`. Use literal
   source → Codex target labels, for example:
...
5. Dry-run, then write the selected target. Use `--replace` only after asking
   the user, because it can delete orphan generated skills or agents.
