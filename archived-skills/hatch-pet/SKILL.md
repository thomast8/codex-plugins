...
- Use one lightweight generation worker per visual job only when the user has
  explicitly asked for or approved delegated workers. Otherwise, run visual jobs
...
- The parent must not open every generated PNG visually. Use worker QA for each
  delegated job and inspect only the final contact sheet.
- After copying the selected generated output into `decoded/`, ask before
  deleting the selected original from `${CODEX_HOME:-$HOME/.codex}/generated_images`
  or removing its now-empty generation directory, unless the user already asked
  for cleanup.
...
If the user provides a brand, company, product, or prospect name rather than a
concrete avatar description or reference image, run lightweight discovery before
preparing the pet run. Use a discovery worker only when the user has explicitly
asked for or approved delegated workers; otherwise do discovery in the parent
thread. Discovery must use web search and prefer official sources such as the
brand site, product pages, docs, about pages, press pages, or brand pages. Use
reputable secondary sources only when official pages are too thin. Keep the
search narrow: enough to extract visual and personality cues, not a
...
- `Getting <Pet> ready.` Choose or confirm the pet name, description, source images, style preset, style notes, and working folder. For bare brand/product/company requests, first run brand discovery and capture the compact brand brief, source URLs, and avatar seed.
...
For brand-only requests, run discovery first, save the markdown brief, then pass the brief path through `--brand-discovery-file`, `avatar_seed` through `--pet-notes`, `brand_name` through `--brand-name`, `brand_brief` through `--brand-brief`, and each source URL through repeated `--brand-source`.
...
- If delegated workers are approved, use one lightweight worker per visual job.
  Otherwise, run these jobs sequentially in the parent thread.
...
Keep image handling minimal: do not open every generated base or row in the
parent rollout. When workers are used, they return only the selected source path
and a one-sentence QA note; the parent records the selected source path in the
manifest.
...
If the copied source is under `${CODEX_HOME:-$HOME/.codex}/generated_images`, ask
before deleting the original generated file after the decoded copy exists:

```bash
# Only run after the user has approved cleanup.
...
`qa/previews/*.gif` before accepting the pet. Use a lightweight visual QA worker
only when delegated workers are approved; otherwise inspect them in the parent
thread. Deterministic validation is necessary but not sufficient. Block
acceptance if any row changes species/body type, face, markings, palette,
material, prop design, style, prop side unexpectedly, or overall silhouette.
Motion previews must also reject unintended size popping, reversed or stagnant
directional cadence, wrong facing direction, and idle loops that are technically
different but visually inert.

After model visual QA accepts the contact sheet, ask before removing intermediate
run artifacts unless the user already asked for cleanup:

Keep `pet_request.json`, `final/spritesheet.webp`, `final/validation.json`,
`qa/contact-sheet.png`, `qa/previews/`, `qa/review.json`, and
`qa/run-summary.json`. After approval, remove generated prompt files, layout
guides, decoded row strips, extracted frames, `final/spritesheet.png`, and the
imagegen job manifest. Skip cleanup when the user wants debug artifacts or the
run still needs repair.

## Lightweight Visual Workers

Use lightweight workers for image-heavy work only when the user explicitly asks
for or approves delegation. This bounds each `$imagegen` rollout to one selected
image, keeps contact-sheet vision payloads out of the parent thread, and reduces
cost while preserving the full 9-state app contract.

## Subagent Delegation

Do not spawn subagents unless the user explicitly asks for or approves them. If
delegation is useful but not yet approved, ask once. If the user declines or does
not answer, run the same workflow sequentially in the parent thread.

Parent responsibilities:

- run brand discovery before preparation when the user provides a bare brand/product/company/prospect name
- prepare the run and inspect `imagegen-jobs.json`
- assign the base job, row jobs, and final contact-sheet QA to lightweight workers only when delegation is approved
...
- Use lightweight visual workers for base generation, row-strip visual generation, and final contact-sheet QA only when delegation is approved; the parent owns manifest updates, deterministic image scripts, packaging, and cleanup.
...
- Contact sheet and per-row motion previews have been produced and inspected by a lightweight visual QA worker or by the parent thread.
